...
@Getter
@Setter
public class DutchBoardPagingCreatorDTO {

	private DutchBoardPagingDTO dutchPaging ; 
	...
	public DutchBoardPagingCreatorDTO(long rowTotal, 
			                     DutchBoardPagingDTO dutchPaging, 
			                     List<DutchBoardVO> dutchlist) {


		this.rowTotal = rowTotal ;
		this.dutchPaging = dutchPaging ;
		this.dutchlist = dutchlist ;
		this.pagingNumCnt = 10 ;	

		//계산된 끝 페이징 번호:
		this.endPagingNum = (int) Math.ceil((double) dutchPaging.getPageNum()/this.pagingNumCnt) * this.pagingNumCnt;

		//계산된 시작 페이징 번호:
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1) ;

		//총 페이지 수 = 맨 마지막 페이지번호
		this.lastPageNum = (int) Math.ceil((double)this.rowTotal / this.dutchPaging.getRowAmountPerPage());
...
//		System.out.println("전달된 페이징 기본데이터-myBoardPagingDTO: " + this.dutchPaging.toString());
