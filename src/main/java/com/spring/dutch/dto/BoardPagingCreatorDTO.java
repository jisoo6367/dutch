...
public class BoardPagingCreatorDTO {
	...
	private BoardPagingDTO boardPaging ;
	...
	public BoardPagingCreatorDTO(long rowTotal,
		...
		//계산 된 끝 페이징 번호:
		this.endPagingNum = (int) Math.ceil((double)boardPaging.getPageNum()/this.pagingNumCnt) * this.pagingNumCnt;

		//계산된 시작 페이징 번호:
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1) ;

		//총 페이지수 = 맨 마지막 페이지 번호
		this.lastPageNum = (int) Math.ceil((double)this.rowTotal / this.boardPaging.getRowAmountPerPage());
