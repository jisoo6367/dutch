...
public class CommunityPagingCreatorDTO {
	...
	private CommunityPagingDTO communityPaging;
	...
	public CommunityPagingCreatorDTO(long rowTotal,
									 CommunityPagingDTO communityPaging,
									 List<CommunityVO> communityList){
		
		this.rowTotal = rowTotal ;
		this.communityPaging = communityPaging;
		this.communityList = communityList ;
		this.pagingNumCnt = 10 ;
		
		//계산된 끝 페이징 번호:
		this.endPagingNum = (int) Math.ceil((double) communityPaging.getPageNum()/this.pagingNumCnt) * this.pagingNumCnt;
		
		//계산된 시작 페이징 번호:
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1) ;
		
		//총 페이지 수 = 맨 마지막 페이징번호
		this.lastPageNum = (int) Math.ceil((double)this.rowTotal / this.communityPaging.getRowAmountPerPage());
