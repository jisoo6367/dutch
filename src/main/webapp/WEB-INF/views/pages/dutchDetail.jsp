...
<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }"/> <!-- 403 forbidden 에러 안나도록 -->
    <div class="row">
        <div class="col-lg-12">
            <h3 class="page-header" style="white-space: nowrap;" >Dutchpay Detail
				 <small>
				 	&nbsp;&nbsp;&nbsp;<c:out value="${dto.dutchboard.pno}"/>번 게시물
				 </small>
			</h3>
        </div><%-- /.col-lg-12 --%>
    </div><%-- /.row --%>
        ...
        <div class="col-lg-12">
    		<div class="panel panel-default">
                <div class="panel-heading">
                	<div class="row">
						<div class="col-md-2" style="white-space: nowrap; height: 45px; padding-top:11px;">
							<strong style="font-size:16px;">${dto.dutchboard.nickname} 님 작성</strong>
						</div>
						<div class="col-md-3" style="white-space: nowrap; height: 45px; padding-top:16px;">
							<span class="text-primary" style="font-size: smaller; height: 45px; padding-top: 19px;">
								<span>
									<span>등록일:&nbsp;</span>
									<strong><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${dto.dutchboard.pregDate}"/></strong>
									<span>&nbsp;&nbsp;</span>
								</span>
								<span>카테고리:&nbsp;<strong><c:out value="${dto.dutchboard.category}"/></strong></span>
							</span>
						</div>
								...
								<c:if test="${username == dto.dutchboard.nickname }"> 
									...
									<c:when test="${dto.dutchboard.pcalculated == 0 }">
										...
										<button type="button" id="btnUpdatePcal" data-oper="modify" 
											class="btn btn-success"><span>정산완료</span></button>
									</c:when>
									</c:choose>
									<button type="button" id="btnToDelete" data-oper="delete"
											class="btn btn-danger"><span>삭제</span></button>
				...
				<div class="panel-body form-horizontal">   <!-- panel-body -->  
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-10">
					    	<textarea class="form-control" rows="3" name="pcontent" id="pcontent"
					    			  style="resize: none;"
					    			  readonly="readonly"><c:out value="${dto.dutchboard.pcontent}"/></textarea>
						</div>	    			  
					</div>
					
					<div class="form-group">
	                  <label class="col-sm-2 control-label" style="white-space: nowrap;">참여자 목록</label>
	                  	<div class="col-sm-10">
						<c:forEach var="user" items="${dto.partiList }">
							<c:if test="${user.nickname == dto.dutchboard.nickname}">
							<input class="form-control" name="partiMember" id="partiMember" value="${user.nickname} (총무)" readonly="readonly"/>
							</c:if>
							<c:if test="${user.nickname != dto.dutchboard.nickname}">
							<input class="form-control" name="partiMember" id="partiMember" value="${user.nickname}" readonly="readonly"/>
							</c:if>
						</c:forEach>
						</div>
	                </div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-10">
					    	<input class="form-control" name="ptotalPayment" id="ptotalPayment" 
					    		   value="${dto.dutchboard.ptotalPayment }" readonly="readonly">
						</div>
					</div>
					...
					<div class="form-group">
					    <label class="col-sm-2 control-label" style="white-space: nowrap;">최종 수정일</label>
					    <div class="col-sm-10">
					    	<input class="form-control" name="bmodDate" id="bmodDate" 
					      		   value='<fmt:formatDate pattern="yyyy/MM/dd HH:mm:ss" value="${dto.dutchboard.pmodDate }"/>' readonly="readonly">
						</div>
					</div>
				</div><%-- /.panel-body --%>
			</div><%--  /.panel --%>
...
<%-- 댓글 목록 표시 영역 - JavaScript로 내용이 생성되어 표시됩니다.--%><%-- 
	<li class="left clearfix commentLi" data-pno="123456" data-rno="12">
		<div>
			<div>
...
<%-- 댓글/답글 자바스크립트 시작--%>
<script type="text/javascript" src="${contextPath }/resources/js/dutchComment.js"></script>
